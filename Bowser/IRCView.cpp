	...
	BString			display,
						...
						URL (int32 os, const char *buffer)
							: offset (os),
  							...
  							url (buffer)
						{
							if (display.Compare ("https://", 8) == 0)
							{
								url  = "http://";
								url += display.String() + 8;
							}
						
							if (display.Compare ("www.", 4) == 0)
							{
								url  = "http://";
								url += display;
							}
...
int32 IRCView::FirstMarker (const char *cData)
{
	...
	do
	{
		urlMarkers[0] = data.FindFirst (tags[0], pos);
		urlMarkers[1] = data.FindFirst (tags[1], pos);
		urlMarkers[2] = data.FindFirst (tags[2], pos);
